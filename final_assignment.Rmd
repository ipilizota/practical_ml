...
```
#### Out of sample error
Thanks to *trControl = fitControl*, it's possible to do:
```{r}
sum(1-modelFit_gbr$results$Accuracy)/length(modelFit_gbr$results$Accuracy)
```
### 2) decision tree  
...
Thanks to *trControl = fitControl*, it's possible to do:
```{r}
sum(1-modelFit_dt$results$Accuracy)/length(modelFit_dt$results$Accuracy)
```
### 3) random forest
...
Thanks to *trControl = fitControl*, it's possible to do:
```{r}
sum(1-modelFit_rf$results$Accuracy)/length(modelFit_rf$results$Accuracy)
```
